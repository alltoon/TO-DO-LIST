...
def middle(row0: int, row1: int, row2: int):
    mid = "╠"
...
def chart(all_row):
    ...
    for row in all_row:
        if row[0]!=1:
            print(middle(count_max_row(0, all_row), count_max_row(1, all_row), count_max_row(2, all_row)))
        print('║ {0} ║ {1} ║ {2} ║ {3} ║'.format(row[0],
                                                 str(row[1]) + str(range_space_cell(count_max_row(1, all_row), row[1])),
                                                 str(row[2]) + str(range_space_cell(count_max_row(2, all_row), row[2])),
                                                 row[3]))
    print(footer(count_max_row(0, all_row), count_max_row(1, all_row), count_max_row(2, all_row)))
